...
- **Turnstile Widget Token Solving**: Beside IUAM `/cf_clearance`, the solver daemon can solve widget tokens via `GET /turnstile?url=<url>&sitekey=<sitekey>`. Inject the returned token into `input[name="cf-turnstile-response"]`.
