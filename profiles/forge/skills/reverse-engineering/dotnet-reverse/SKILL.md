---
name: dotnet-reverse
description: Reverse engineering biner .NET / C#. Gunakan saat target adalah assembly .NET (header PE berisi CLR, executable/dll terkelola), hasil kompilasi C# (termasuk NativeAOT), tool Red Team Sharp* (Rubeus / SharpHound, dll.), obfuscator .NET (ConfuserEx / SmartAssembly / Babel / Eazfuscator), loader .NET / info-stealer / malware pembungkus. Utamakan dnSpyEx + de4dot. Untuk biner native murni, gunakan reverse-engineering / ida-reverse.
license: MIT
compatibility: Membutuhkan code agent berbasis filesystem atau CLI dengan akses shell, diutamakan Windows host (dnSpyEx adalah GUI Windows); Linux/macOS dapat menggunakan ILSpy/de4dot CLI + mono/dotnet runtime.
