...
- **Session Locks & Running Background Processes:** SQLite session files (`.session`) are strictly single-process locked. When working on Telethon accounts, running background runner processes will hold a process-lock on the `.session` file (causing `sqlite3.OperationalError: database is locked`). To inspect metadata safely without interrupting the active process, connect in read-only URI mode: `sqlite3.connect('file:/path/to/session.session?mode=ro', uri=True)`. To reconnect or run standalone tasks, check for locking processes via `fuser <path_to.session>` and terminate them (`kill <PID>` or `fuser -k <path_to.session>`) before running the script.
