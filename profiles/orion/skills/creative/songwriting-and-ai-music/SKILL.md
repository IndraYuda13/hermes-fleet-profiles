...
## 8. Local/open-source generation choice

Use the songwriting workflow above to prepare lyrics, structure, vocal direction, and a dynamic prompt; then choose a generator by the output needed:

- **Hosted lyric-led generation (for example Suno):** use Custom Mode, structural tags, a vocal persona, and several takes.
- **Local HeartMuLa:** full lyric-and-tag-conditioned songs when a CUDA machine has roughly 8 GB VRAM with lazy loading (16 GB preferred). Preserve codec fp32; its upstream dependency/transformer patches and commands are in `references/heartmula-local-generation.md`.
- **AudioCraft/MusicGen or AudioGen:** short instrumental/sound-design material, melody/style conditioning, or sound effects; it is not a substitute for HeartMuLa's lyric-first full-song workflow.

For any generator, audition at least a short test output before investing in long renders; revise pronunciation, tags, and arrangement instructions before regenerating.
