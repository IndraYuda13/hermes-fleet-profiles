...
## 8. Real-Time Live Chat & Host-Aware Routing Pattern (Express + Socket.IO + SQLite)

When building self-hosted customer communication / live chat tools (Crisp/Intercom alternative) with separate customer & admin entrypoints:

1. **Host-Aware Domain Dispatching**:
   - Differentiate incoming requests based on `req.hostname` or `Host` header to serve dedicated views on a single Node.js service:
   ```javascript
   app.use((req, res, next) => {
     const host = req.hostname || req.headers.host || '';
     if (host.startsWith('admin.')) {
       req.isSubdomainAdmin = true;
     }
     next();
   });
   ```
2. **Room Isolation & Socket Events**:
   - Customer sockets join dedicated rooms by persistent `session_id` (`session:<session_id>`).
   - Admin sockets join `admin_channel` to receive broadcasts of incoming customer messages and visitor telemetry.
   - Keep messages stored synchronously in SQLite with indexing on `(session_id, created_at)`.
3. **MVP Scope Discipline**:
   - When building real-time interactive MVPs, prioritize core messaging, typing indicator, unread count, and telemetry first.
   - Defer complex auxiliary features (file uploads, multi-role auth, canned replies, auto-bots) to an explicit backlog/TODO to ensure fast delivery of working interactive prototypes.
   - See detailed architecture and audio synth helper in `references/livechat-architecture.md`.

---
## 9. Real-Time Pari-Mutuel Game Loop & Provably Fair Settlement

For timed round betting/duel games (e.g. 60s Ganjil/Genap parity games) with zero-opponent refund invariants, 95% enemy pool splits, and SHA-256 seed pre-commitments:
- Detailed state machine, exact mathematical formulas, and provably fair hashing patterns are documented in `references/parimutuel-gameloop.md`.

---

## 10. SRE Incident Telemetry & Lifecycle State Engine (Express + SQLite)

When implementing internal incident monitoring, telemetry metrics, and lifecycle tracking:

1. **State Machine & Status Flow**:
   - Status transitions: `investigating` -> `identified` -> `monitoring` -> `resolved`.
   - Update timeline history on every state transition (`timeline_events` table) with timestamp, actor, and message.
2. **Payload Sanitization & Strict Type Validation**:
   - Prevent runtime type crashes on POST/PATCH endpoints by explicitly asserting `typeof` string and enum bounds for `severity` (`P1`, `P2`, `P3`, `P4`) and `status`:
   ```javascript
   const validSeverities = ['P1', 'P2', 'P3', 'P4'];
   if (!severity || typeof severity !== 'string' || !validSeverities.includes(severity.trim().toUpperCase())) {
     return res.status(400).json({ error: 'Invalid severity. Must be one of P1, P2, P3, P4' });
   }
   ```
3. **Collision-Resistant ID Generation under Rapid Concurrent Inserts**:
   - Using `MAX(id)` or regex parsing on previous rows (`SELECT id FROM incidents WHERE id LIKE 'INC-%'`) can result in duplicate primary key collisions (`SQLITE_CONSTRAINT_PRIMARYKEY`) if concurrent requests execute before transactions commit.
   - **Pattern**: Ensure ID generation checks for existing collision and increments in a loop, or use UUIDv4/CUID/NanoID for primary keys while keeping sequential ticket numbering in a dedicated sequence counter table.
   ```javascript
   function generateIncidentId() {
     const rows = db.prepare(`SELECT id FROM incidents WHERE id LIKE 'INC-%'`).all();
     let maxNum = 1000;
     for (const r of rows) {
       const match = r.id.match(/^INC-(\d+)$/);
       if (match) {
         const num = parseInt(match[1], 10);
         if (num > maxNum) maxNum = num;
       }
     }
     let nextId = `INC-${maxNum + 1}`;
     while (db.prepare('SELECT id FROM incidents WHERE id = ?').get(nextId)) {
       maxNum++;
       nextId = `INC-${maxNum + 1}`;
     }
     return nextId;
   ...
   ```
4. **Telemetry Metrics & MTTR Engine**:
   - Compute MTTR in minutes from resolved incidents: `(SUM(julianday(resolved_at) - julianday(created_at)) * 1440) / COUNT(resolved_incidents)`.
   - Calculate real-time System Health Score (e.g., base 100 with severity penalty: P1 -35%, P2 -15%, P3 -5%, P4 -1%).
