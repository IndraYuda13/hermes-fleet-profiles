...
- **Test State & Persistent Cache Isolation:** Unit and regression test suites touching shared data layers (e.g. MySQL `product_cache`, SQLite stores, Redis) must strictly use isolated test databases, mocks, or transactional rollbacks. Never let test runs overwrite live production caches with truncated mock fixtures without automatic state restoration.
...
- **Independent review & QA separation:** For consequential code changes, use a fresh reviewer context and require evidence-backed findings. Keep any fix loop bounded and re-run the same checks after each change. Reviewer/QA agents MUST NOT modify production code during an acceptance review (`production_files_modified_by_reviewer == 0`); any discovered defect must be routed back to the implementer for remediation in a new Git revision.
