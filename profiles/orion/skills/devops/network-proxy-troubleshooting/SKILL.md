...
### Real-Time SPA Input Forms & Browser Navigation 404s
When developing or debugging single-page apps (SPAs), live chat widgets, or WebSocket frontends behind reverse proxies or Cloudflare Tunnel:
- **Symptom:** User types in an input field, presses `Enter`, and the page navigates to `https://subdomain.domain.com/?` with a misleading `HTTP ERROR 404` or drops the active WebSocket connection.
- **Root Cause:** The input is wrapped in a `<form>` element without preventing default submit actions. Pressing Enter executes a native HTML form GET request (`GET /?`), causing page reloads and tearing down real-time socket connections.
- **Fix:** Avoid `<form>` wrappers for real-time socket inputs or attach explicit `e.preventDefault()` inside `keydown` (Enter) and button `click` handlers.
