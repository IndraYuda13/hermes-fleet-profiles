...
- **Docker JSON Log Truncation**: Docker container stdout/stderr log files in `/var/lib/docker/containers/<id>/<id>-json.log` can grow to hundreds of megabytes if log rotation is missing. Truncate active log files using `truncate -s 0 /var/lib/docker/containers/*/*-json.log` (along with `/var/log/syslog` and `/var/log/btmp`) to reclaim space safely without stopping containers.
- **Selective Service Decommissioning for Refactoring**: When taking dockerized applications offline for major UI/UX or backend refactoring (e.g., Finance app, Open-WebUI), stop and remove containers (`docker stop` & `docker rm`), delete their docker images (`docker rmi`), and run `docker image prune -af`. Always confirm that project source code directories outside Docker volumes are preserved.
...
## Quick VPS status snapshot & Docker Container Audit
...
When sorting active Docker containers by image footprint or size (excluding specific proxies or fleets when requested):
```python
import subprocess
out = subprocess.check_output(["docker", "ps", "--format", "{{.Names}}\t{{.Image}}\t{{.Status}}"]).decode()
containers = []
for line in out.strip().split("\n"):
    if not line.strip(): continue
    parts = line.split("\t")
    name, image, status = parts[0], parts[1], parts[2]
    if "surfshark" in name: continue  # Exclude per user preference
    try:
        size_bytes = int(subprocess.check_output(["docker", "image", "inspect", image, "--format", "{{.Size}}"]).decode().strip())
    except Exception:
        size_bytes = 0
    containers.append((name, image, size_bytes, status))

containers.sort(key=lambda x: x[2], reverse=True)
for c in containers:
    size_str = f"{c[2] / (1024*1024*1024):.2f} GB" if c[2] >= 1024*1024*1024 else f"{c[2] / (1024*1024):.1f} MB"
    print(f"{c[0]:<32} | {c[1]:<40} | {size_str:<12} | {c[3]}")
```
...
### Live Chat & WebSocket Shared Inbox Services
When running real-time customer support apps (Crisp/Intercom equivalents) on Node.js + SQLite:
- Use WAL mode (`db.pragma('journal_mode = WAL')`) for high concurrency message reads/writes.
- Keep session join logic inside `socket.on('connect')` so that reconnects restore room bindings automatically.
- Frontends must use optimistic rendering with temporary message IDs and status transitions (`sending` -> `delivered` -> `read`).
- Shared inboxes must auto-select active sessions and sort by `last_message_at DESC` to prevent lost/unaddressed replies.
