...
- **Docker Builder Cache (BuildKit/legacy build cache)**: Often holds tens of GBs of forgotten build layers.
  ...
  ```
- **Docker Container Log Truncation**: Un-rotated container stdout/stderr logs in `/var/lib/docker/containers/` can grow to hundreds of MBs. Check and safely truncate them without stopping containers:
  ```bash
  # Find largest container log files
  du -sh /var/lib/docker/containers/*/*-json.log 2>/dev/null | sort -hr | head -n 10

  # Truncate specific bloated log file to 0 bytes
...
uv cache clean 2>/dev/null || rm -rf ~/.cache/uv /root/.hermes/profiles/*/home/.cache/uv

# Clean HuggingFace model cache across multi-agent profiles
rm -rf ~/.cache/huggingface /root/.hermes/profiles/*/home/.cache/huggingface

# Clean user/tool caches (always recreate ~/.cache after purging so tools don't break)
rm -rf ~/.cache && mkdir -p ~/.cache
rm -rf ~/.npm/_cacache /root/.hermes/profiles/*/home/.npm/_cacache
npm cache clean --force

# Clean old fleet migration/upgrade backup tarballs (keep only latest)
# e.g. /root/.hermes/backups/
ls -lh /root/.hermes/backups/
# rm -rf /root/.hermes/backups/<stale_full_backup_tarball_or_dir>

# Clean decompilation, offload & temp directories
rm -rf /root/apk_extract /mnt/openclaw-offload /tmp/jadx_* /tmp/firecrawl /tmp/* /var/tmp/*
```

#### Bot Database Backup Rotation & Pruning
When automated micro-backups (e.g. database dump bots writing SQL files every few minutes) accumulate thousands of un-rotated files:
- Inspect file count and size: `ls -1 <backup_dir> | wc -l`
- Prune stale historical dumps older than retention window (e.g., keeping only the last 14 days):
  ```bash
  find /path/to/backups/ -name "*.sql" -mtime +14 -delete
  ```
- Re-check disk gain and remaining snapshots: `ls -1 <backup_dir> | wc -l && df -h /`

#### Browser Automation State & Code Caches
Agent frameworks (Playwright, Puppeteer, OpenClaw browser profiles) generate multi-hundred megabyte ephemeral Chromium caches (`Default/Cache`, `Default/Code Cache`, `Default/GPUCache`) in session state trees without affecting stored auth cookies/tokens:
- Safely clear Chromium caches across browser profile directories:
  ```bash
  find /path/to/state/*browser*/Default/Cache -type f -delete 2>/dev/null
  find /path/to/state/*browser*/Default/Code\ Cache -type f -delete 2>/dev/null
  find /path/to/state/*browser*/Default/GPUCache -type f -delete 2>/dev/null
  ```
