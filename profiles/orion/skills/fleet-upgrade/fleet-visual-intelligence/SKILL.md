...
### Orion Marginal Value Rule & Zero-Implementation Boundary
...
**ORION Zero-Implementation Boundary:** ORION is strictly an orchestrator and quality governor. When backend/logic or UI defects emerge during mission verification, ORION must NEVER edit production files directly (`edits=0`). Always spawn a remediation Kanban task for the responsible specialist (FORGE for backend/DB, FRAME for frontend UI, ATLAS for deployment).
...
### Output Deliverable
Produce `VISUAL_QA_REPORT.md` containing: Revision SHA, tested routes/states, viewports, screenshot artifacts, correctness defects, art direction defects, generic risk rating, console/network audit, and final verdict. See `references/mobile-responsive-remediation-patterns.md` for mobile layout overflow prevention and table container styling.
