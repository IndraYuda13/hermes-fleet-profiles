...
## Exploratory sketches before commitment

When the user wants to compare UI directions rather than receive production-ready implementation, build two or three disposable, complete HTML variants that differ by a real design stance—density, hierarchy, layout, or aesthetic—not by a color swap. Make the primary action clickable, verify each variant visually, and present an opinionated trade-off table before promoting a winner into a polished artifact. The full intake, variation, and browser-verification recipe is in `references/exploratory-sketches.md`.
