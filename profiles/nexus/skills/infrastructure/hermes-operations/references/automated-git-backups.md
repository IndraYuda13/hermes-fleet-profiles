...
- **GitHub 100MB Limit & Git LFS:** GitHub blocks pushes containing files >100MB (like `state.db`). If backing up large SQLite `.db` files, track them with Git LFS (`git lfs track "*.db"`) and run `git lfs migrate import --include="*.db" --include-ref=master` if large commits already exist in history.
