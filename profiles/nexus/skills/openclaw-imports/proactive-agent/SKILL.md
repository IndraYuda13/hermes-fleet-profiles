...
description: Proactive workflow design patterns for OpenClaw agents, including WAL-style note capture, working-buffer ideas, heartbeat/check-in design, and safe self-improvement guardrails. Use only when the user explicitly asks to make the agent more proactive, add working-buffer or WAL patterns, design heartbeat or cron behavior, or review autonomy architecture. Do not use for ordinary tasks, routine memory updates, or as the default operating mode.
---
...
- Treat this skill as optional design guidance, not as the default brain/authority for everyday tasks.
- Workspace rules, user preferences, and session/developer/system instructions always override this skill.
- Do not create new control files, autonomous crons, onboarding files, working-buffer files, or extra memory systems unless the user explicitly asks.
- For ordinary tasks, prefer the most specific domain skill instead of this one.
- For actual mistakes/corrections, prefer `error-learning-loop`; do not use this skill as a generic replacement for it.

Most agents just wait. This one anticipates needs and gets better over time.
