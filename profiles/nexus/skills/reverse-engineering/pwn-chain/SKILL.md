...
name: pwn-chain
description: |
  Metodologi rekayasa alur penuh dari reverse engineering hingga eksploitasi aktif (Working Exploit).
  Skenario penggunaan: Mendapatkan biner + titik kerentanan + lingkungan target, dan perlu menulis script exploit yang stabil (bukan sekadar PoC lokal yang gagal di lingkungan remote).
  Mencakup 3 arah utama: Stack Overflow / Heap Exploitation / Kernel Pwn. Menekankan penanganan perbedaan antara CTF lokal vs target remote nyata (mismatch versi libc, timing heap spray, SMEP/SMAP/KASLR, alignment stack, buffering remote).
  Toolchain utama: pwntools + GEF/pwndbg + ROPgadget/Ropper + one_gadget + libc-database + qemu-system kernel debugging.
  Kata kunci pemicu: pwn, stack overflow, heap overflow, ROP, ret2libc, ret2csu, one_gadget, libc-database, eksploitasi heap, tcache, fastbin, unsorted bin, kernel pwn, kROP, SMEP, SMAP, KASLR, modprobe_path, pwntools, GEF, pwndbg.
---
