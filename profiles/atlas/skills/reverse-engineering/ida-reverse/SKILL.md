...
name: ida-reverse
description: |
  Skill pembantu analisis reverse engineering IDA Pro. Gunakan skill ini saat melakukan analisis biner/PE/ELF/APK/DLL/SO, decompilation, analisis kerentanan, analisis malware, analisis firmware, atau kebutuhan analisis file executable (.exe/.dll/.so/.elf/.macho/.sys).

  Gunakan skill ini ketika pengguna ingin menganalisis file biner apa pun, baik menyebutkan "IDA" / "reverse engineering" secara eksplisit maupun tidak.

  Gunakan skrip bawaan (scripts/start.ps1, scripts/open.ps1) untuk manajemen server secara deterministik.
---
