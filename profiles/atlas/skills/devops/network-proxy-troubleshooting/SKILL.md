...
- **Remediation:**
  ...
  4. **Content-Security-Policy (CSP) with Cloudflare Web Analytics / Insights:**
     - When Cloudflare Web Analytics / Real User Monitoring (RUM) is enabled on a proxied domain, Cloudflare automatically injects `<script defer src='https://static.cloudflareinsights.com/beacon.min.js' ...></script>` and beacons telemetry back to `https://cloudflareinsights.com` and `https://static.cloudflareinsights.com`.
     - An origin Nginx with strict `Content-Security-Policy: default-src 'self'; script-src 'self';` blocks this beacon script and telemetry report at the browser console level.
     - Solution: explicitly include `https://static.cloudflareinsights.com` in `script-src` and both `https://cloudflareinsights.com https://static.cloudflareinsights.com` in `connect-src`. If `add_header` is declared inside `location /`, update both the `server` block and child `location` blocks to prevent header inheritance shadowing.
  5. **Default Favicon / Asset 404 in SPA Deployments:**
     - Modern SPA templates (e.g. Vite default scaffold) reference `<link rel="icon" type="image/svg+xml" href="/vite.svg" />` or `/favicon.ico` in `index.html`.
     - When purging dev assets or replacing template files, ensure a matching icon exists in both `public/` and `dist/` before deployment. Otherwise, browsers issue an automatic `/vite.svg` or `/favicon.ico` fetch that results in a console 404 on live production audits.
  6. **Query String Cache-Busting:** Inject explicit versioning parameters into HTML asset tags (e.g. `/js/app.js?v=2.0.0`, `/css/styles.css?v=2.0.0`) to immediately force Cloudflare edge cache misses (`cf-cache-status: BYPASS`) and mobile browser cache eviction without requiring Cloudflare API purge keys.
